...
class WizDocument(object):
    ...
    def resolve_attachments(self, attachments: list[WizAttachment], skip_missing_attachments: bool = False) -> None:
        ...
        try:
            ...
            if skip_missing_attachments:
                # 如果设置了跳过缺失附件，则记录日志并继续
                logger.warning(f'{e!s}，文档 {self.title} 的附件缺失，已跳过')
                # 从附件列表中移除缺失的附件
                self.attachments = [a for a in self.attachments if a.file.exists()]
            else:
    ...
    def _parse_wiz_note(self, skip_missing_images: bool = False) -> None:
        ...
        self.body, self.internal_links, self.images = parse_wiz_html(self.note_extract_dir, self.title, skip_missing_images)
    ...
    def resolve_body(self, skip_missing_images: bool = False) -> None:
        ...
        self.check_note_file()
        self._extract_zip()
        self._parse_wiz_note(skip_missing_images)

    def resolve(self, attachments: list[WizAttachment], tags: list[WizTag], skip_missing_attachments: bool = False, skip_missing_images: bool = False) -> None:
        self.resolve_attachments(attachments, skip_missing_attachments)
        self.resolve_tags(tags)
        self.resolve_body(skip_missing_images)
...
class WizStorage(object):
    ...
    def build_documents(self, skip_missing_attachments: bool = False, skip_missing_images: bool = False) -> list[WizDocument]:
        ...
        for row in rows:
            ...
            document.resolve(
                ...
                skip_missing_attachments,
                skip_missing_images
            )
            documents.append(document)
        return documents

    def build_document(self, guid: str, skip_missing_attachments: bool = False, skip_missing_images: bool = False) -> WizDocument:
        """ 构建一个 document
        """
        ...
        document.resolve(attachments, tags, skip_missing_attachments, skip_missing_images)
        return document

    def resolve(self, skip_missing_attachments: bool = False, skip_missing_images: bool = False) -> None:
        """ 解析所有文档并保存相关数据
        调用此方法后，所有数据安全并可用
        """
        self.documents = self.build_documents(skip_missing_attachments, skip_missing_images)
