...
class WizImage(object):
    ...
    def __init__(self, outerhtml: str, src: str, note_extract_dir: Path, skip_missing_images: bool = False) -> None:
        self.outerhtml = outerhtml
        self.src = src
        ...
        if not self.file.exists():
            if skip_missing_images:
                # 如果设置了跳过缺失图片，则记录日志并继续
                logger.warning(f'找不到图片文件 {self.file}，已跳过')
                # 设置一个标志，表示这个图片文件缺失
                self.missing = True
            else:
...
def parse_wiz_html(note_extract_dir: Path, title: str, skip_missing_images: bool = False) -> tuple[str, list[WizInternalLink], list[WizImage]]:
    """ 在为知笔记文档的 index.html 中搜索内链的附件和文档链接
    """
    index_html = note_extract_dir.joinpath('index.html')
    if not index_html.is_file:
        raise FileNotFoundError(f'找不到文件 {index_html}！')
    ...
    for image in image_match:
        img = WizImage(image.group(0), image.group(1), note_extract_dir, skip_missing_images)
        images.append(img)
